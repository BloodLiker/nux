...
def urbanDictionary(bot):
    ...
    if message.startswith('!ud '):
        ...
        data = http.gethttp('http://www.urbandictionary.com/define.php?term='+urllib2.quote(message[4:]))
        ...
        try:
            print definition
            print
            bot.response(http.unescape(definition.decode('ascii', 'ignore')))
            #print '\n'+http.unescape(repr(definition))+'\n'
            #bot.response(message[4:].decode('?????????????')+' - '+http.unescape(definition.decode('ascii', 'ignore')))
        except:
            traceback.print_exc()
            bot.error('PROBLEM WHILE UNESCAPING HTML')
            bot.response(message[4:]+' - '+definition)
