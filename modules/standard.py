...
def timenow(bot):
    if bot.getMessage() == '!time':
        bot.response(time.strftime('%T'))
...
def auth(bot):
    if bot.isGod(bot.getSenderNick()) and bot.getMessage() == '!auth':
        bot.auth()

def CTCPPing(bot):
    if bot.getMessage().startswith('\001PING'):
        bot.notice(bot.getSenderNick(), '\001PING'+bot.getMessage()[5:])

def CTCPVersion(bot):
    if bot.getMessage().startswith('\001VERSION'):
        bot.notice(bot.getSenderNick(), '\001VERSION '+bot.getVersion()+'\001')

functions = [timenow, ping, CTCPPing, CTCPVersion, join, invite, part, quit, mode, msg, quote, god, auth]
